    ...
    if (!error.response) {
        ...
        switch (error.response.data.subCode) {
            ...
            case 101://Host application's architecture is 64Bit but source is 32Bit
                ...
                break;
            case 102://Couldn't find DAC file P
                window.vueInstance.$errNotifier("Error", "The host application couldn't found DAC file. Possible dump file corruption. Error reported to the support team.");
                break;
            default:
                window.vueInstance.$errNotifier("Error Occured", error.response.data.message);
                break;
